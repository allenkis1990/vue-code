<template>
  <div class="container weui-tab">
      ...
      <div class="tab-bd mt10" v-if="pageState.deliverType == 1">
        <div class="weui-panel hb-panel">
          <div class="weui-cells weui-cells_form">
            ...
            <!-- <div class="weui-cell weui-cell_select weui-cell_select-1 weui-cell_select-after">
              <div class="weui-cell__hd">
                <label class="weui-label"><i class="star">*</i>收件地区</label>
              </div>
              <div  class="weui-cell__bd">
    ...
    <div class="weui-tabbar hb-tabbar">
      <div @click="saveInvoiceDeliverInfo()" class="hb-btn">保 存</div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    saveInvoiceDeliverInfo: function () {
      // 1 邮寄  2 自取
      if (this.pageState.deliverType === 1) {
        // 赋值
        this.invoiceInfo.deliverType = this.pageState.deliverType
        this.invoiceInfo.receiverAddressId = this.userAddressInfo.id
        this.invoiceInfo.addressDetails = this.userAddressInfo.addressDetails
        ...
        if (this.checkInformation()) {
          ...
          this.$store.dispatch(storeTypes.ACCOUNT_UPDATE_RECEIPTINFO, this.userAddressInfo).then((data) => {
            if (data.head.code !== storeTypes.NETWORK_RESULT_SUCCESS) {
              ...
              this.invoiceInfo.receiverAddressId = data.data.id
              this.$store.dispatch(storeTypes.ORDER_SET_COMPLETE_TEMP_INVOICE_INFO, this.invoiceInfo)
              this.back()
            }
          }).catch(e => {
        ...
        if (this.selfPickAddressList.length === 0) {
          this.toast('未选择自取地址')
          return
        }
        // 验证
        // 提交自取 存入store
        this.invoiceInfo.deliverType = this.pageState.deliverType
        this.invoiceInfo.selfStorageId = this.selfPickAddressList[this.currentSelfStorageIndex].selfStorageId
        this.invoiceInfo.selfStorageName = this.selfPickAddressList[this.currentSelfStorageIndex].selfStorageName
        this.$store.dispatch(storeTypes.ORDER_SET_COMPLETE_TEMP_INVOICE_INFO, this.invoiceInfo)
        this.back()
      }
    },
    ...
    requestSelfPickAddress: function () {
      ...
      this.$store.dispatch(storeTypes.ORDER_GET_SELF_PICK_ADDRESS, {type: 1}).then(data => {
        this.hideLoading()
        if (data.head.code === storeTypes.NETWORK_RESULT_SUCCESS) {
          ...
          for (var i = 0; i < this.selfPickAddressList.length; i++) {
            ...
            if (item.selfStorageId === this.invoiceInfo.selfStorageId) {
    ...
    requestUserAddress: function () {
      ...
      this.$store.dispatch(storeTypes.ACCOUNT_GET_RECEIPTINFO, {}).then(data => {
        if (data.head.code === storeTypes.NETWORK_RESULT_SUCCESS) {
          that.userAddressInfo = data.data
          that.areaValue.push(data.data.provinceId)
          that.areaValue.push(data.data.cityId)
          that.areaValue.push(data.data.districtId)
        } else {
          console.log(JSON.stringify(data))
        }
      }).then(e => {
