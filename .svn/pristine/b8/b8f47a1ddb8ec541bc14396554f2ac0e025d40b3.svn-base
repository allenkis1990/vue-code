<template>
  <div class="weui-panel hb-panel bor-b20" @click="cellClickAction">
    ...
    <order-list-sub-cell v-if="item.subOrderList.length <= 3" v-for="subItem in item.subOrderList" :key="subItem.subOrderNo" :subItem="subItem" :channel="item.channel" :overThree="false" @on-subSeeRefund="seeRefundAction(subItem)"  @on-subChangeClassRecord="changeClassRecordAction(subItem)" ></order-list-sub-cell>
    ...
    <div class="weui-cells" style="margin-top:0" v-if="item.subOrderList.length > 3" @click="goMoreSubOrderList(item.subOrderList, $event)">
      <div class="weui-cell weui-cell_access">
        <div class="weui-cell__bd"></div>
        <div class="weui-cell__ft weui-cell__ft-4">查看更多培训内容</div>
      </div>
    </div>
    ...
    <div class="weui-panel__op bor-t" v-if="isShowBottomBar()" style="padding-top:10px;padding-bottom:0">
      <div class="hb-btn-s" @click="buttonAction('seeRefund', $event)" v-if="isShowSeeRefundButtton()">查看退款</div>
      <div class="hb-btn-s" @click="buttonAction('changeClassRecord', $event)" v-if="isShowChangeClassRecordButton()">换班记录</div>
      <div class="hb-btn-s" @click="buttonAction('relevanceOrder', $event)" v-if="isShowRelevanceOrder()">关联订单</div>
      <div class="hb-btn-s" @click="buttonAction('cancelOrder', $event)" v-if="isShowCancelOrderButton() && item.tradeChannel !== 'COLLECTIVE' && item.tradeChannel !== 'PRESENT'">取消订单</div>
      <div class="hb-btn-s current" @click="buttonAction('goToPay', $event)" v-if="isShowCancelOrderButton() && item.tradeChannel !== 'COLLECTIVE' && item.tradeChannel !== 'PRESENT'">立即支付</div>
      <div class="hb-btn-s" @click="buttonAction('completeDistributionInfo', $event)" v-if="isShowcompleteDistributionInfoButtton()">完善信息</div>
    </div>
  </div>
</template>
<style lang="less">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      isShowcompleteDistributionInfoButtton (item) {
